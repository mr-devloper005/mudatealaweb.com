...
export function Footer() {
  ...
  if (recipe.footer === 'minimal-footer') {
    return (
      <footer className="border-t border-white/10 bg-[#013220] text-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-4 py-8 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div>
            <p className="text-lg font-semibold tracking-tight">{SITE_CONFIG.name}</p>
            <p className="mt-1 text-sm text-white/70">{SITE_CONFIG.description}</p>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-3">
            {enabledTasks.slice(0, 5).map((task) => (
              <Link
                key={task.key}
                href={task.route}
                className="rounded-lg border border-white/20 bg-white/5 px-3 py-2 text-sm font-medium text-white/90 transition-colors hover:border-[#66C2B2] hover:text-[#a8e8dc]"
              >
                {task.label}
              </Link>
            ))}
          ...
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr_1fr]">
            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-7">
              <div className="flex items-center gap-3">
                <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-white/12 bg-white/8 p-2">
        ...
        <div className="grid gap-10 md:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr_0.8fr]">
          <div>
            <Link href="/" className="flex items-center gap-3">
              <div className="h-16 w-16 overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">
                <img src="/favicon.png?v=20260422" alt={`${SITE_CONFIG.name} logo`} width="64" height="64" className="h-full w-full object-contain" />
              </div>
              <div>
